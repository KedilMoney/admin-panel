...
import { Expert, ExpertFormData, FeeModel, TrialSession } from '@/types';
...
/** Inline tag-input. Type a language, press Enter or comma to add it as a chip. */
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 min-h-[40px] w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1.5 focus-within:ring-2 focus-within:ring-ring">
      {values.map((lang, i) => (
        <span
          key={`${lang}-${i}`}
          className="inline-flex items-center gap-1 rounded-md bg-[var(--secondary)] px-2 py-0.5 text-xs font-medium text-[var(--secondary-foreground)]"
        >
          {lang}
          <button
            type="button"
            onClick={() => removeAt(i)}
            disabled={disabled}
            className="text-[var(--muted-foreground)] hover:text-red-600 leading-none"
      ...
      <input
        ...
        onBlur={commitDraft}
        disabled={disabled}
        placeholder={values.length === 0 ? 'Type a language and press Enter' : ''}
...
export default function ExpertsPage() {
  ...
  // Pending file selected in the form but not yet uploaded. Uploaded on submit.
  const [pendingPhotoFile, setPendingPhotoFile] = useState<File | null>(null);
  // Object URL for local preview of pendingPhotoFile (revoked on cleanup).
  const [photoPreview, setPhotoPreview] = useState<string>('');
  // Inline form-level error shown above the action buttons when a mutation fails.
  const [formError, setFormError] = useState<string>('');
  const photoInputRef = useRef<HTMLInputElement>(null);

  // Revoke local preview URLs to avoid memory leaks
  useEffect(() => {
    return () => {
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    // Client-side guardrails — backend enforces the same limits, but failing
    // here gives instant feedback without a wasted round-trip.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormError('');
    try {
      // Two-step submit: upload the photo first (if a new file was selected),
      // then send the create/update payload with the returned URL.
      let payload = form;
      if (pendingPhotoFile) {
      ...
      const apiMessage =
        err?.response?.data?.message || err?.message || 'Failed to save expert';
      setFormError(apiMessage);
      console.error('[experts] submit failed:', err);
    } finally {
        ...
        <AdminLayout>
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="mt-4 text-[var(--muted-foreground)]">Loading experts...</p>
            </div>
          </div>
        </AdminLayout>
      </AuthGuard>
      ...
      <AdminLayout>
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-[var(--foreground)]">Experts</h1>
              <p className="mt-2 text-[var(--muted-foreground)]">Manage the personal finance expert directory</p>
            </div>
          ...
          <Card>
            <CardHeader className="border-b border-[var(--border)]">
              <div className="flex items-center justify-between">
                <CardTitle className="text-xl">All Experts</CardTitle>
                  ...
                  {experts && experts.length > 0 ? (
                    experts.map((expert) => (
                      <TableRow key={expert.id}>
                        ...
                        <TableCell>{expert.specialisation}</TableCell>
                        <TableCell>{expert.city}</TableCell>
                        <TableCell>
                          <Badge variant="outline">{FEE_MODEL_LABELS[expert.feeModel]}</Badge>
                        </TableCell>
                        <TableCell>{expert.certification}</TableCell>
                        <TableCell>
                    ...
                    <TableRow>
                      <TableCell colSpan={10} className="text-center py-12">
                        <div className="flex flex-col items-center gap-2">
                          <p className="text-[var(--muted-foreground)]">No experts yet</p>
        ...
        <Dialog open={dialogOpen} onOpenChange={closeDialog} contentWrapperClassName="max-w-3xl">
          <DialogContent className="max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingExpert ? `Edit: ${editingExpert.name}` : 'Add Expert'}</DialogTitle>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="specialisation" value={form.specialisation} onChange={(e) => set('specialisation', e.target.value)} required disabled={isSubmitting} placeholder="e.g. Debt Management" />
                  </div>
                  <div className="space-y-2">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={form.feeModel} onValueChange={(v) => set('feeModel', v as FeeModel)} disabled={isSubmitting}>
                      <SelectTrigger>
                        <SelectValue placeholder="Select fee model" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="FEE_ONLY">Fee Only</SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="certification" value={form.certification} onChange={(e) => set('certification', e.target.value)} required disabled={isSubmitting} placeholder="e.g. SEBI RIA / CFP / Uncertified" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="registrationNo">Registration No <span className="text-xs text-[var(--muted-foreground)]">(optional)</span></Label>
                    <Input id="registrationNo" value={form.registrationNo} onChange={(e) => set('registrationNo', e.target.value)} disabled={isSubmitting} placeholder="e.g. INA000012345" />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={form.trialSession} onValueChange={(v) => set('trialSession', v as TrialSession)} disabled={isSubmitting}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="FIRST_SESSION">First Session</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2 col-span-2">
                    ...
                    <Input id="website" value={form.website} onChange={(e) => set('website', e.target.value)} disabled={isSubmitting} placeholder="https://expertname.com" />
                  </div>
                </div>
              </div>
              ...
              {/* Agency */}
              <div>
                <h3 className="text-sm font-semibold text-[var(--muted-foreground)] uppercase tracking-wide mb-3">Agency / Firm</h3>
                <div className="space-y-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm">This expert has their own agency or firm</span>
                  </label>
                  ...
                  {form.hasAgency && (
                    <div className="grid grid-cols-2 gap-4 pl-6 border-l-2 border-[var(--border)]">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="agencyWebsite">Agency Website <span className="text-xs text-[var(--muted-foreground)]">(optional)</span></Label>
                        <Input id="agencyWebsite" value={form.agencyWebsite} onChange={(e) => set('agencyWebsite', e.target.value)} disabled={isSubmitting} placeholder="kedil.money or https://kedil.money" />
                      </div>
                      <div className="space-y-2 col-span-2">
                        ...
                        <textarea
                          ...
                          rows={2}
                          maxLength={500}
                          placeholder="Short description of the agency or firm"
                          className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-50"
                        />
                      </div>
                    </div>
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={isSubmitting}>
                  {isSubmitting ? (editingExpert ? 'Saving...' : 'Creating...') : (editingExpert ? 'Save Changes' : 'Create Expert')}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
